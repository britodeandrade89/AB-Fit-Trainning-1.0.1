...
function initializeDatabase() {
    ...
    const periodizacaoData = [
        { week: "1ª e 2ª Semanas", phase: "Adaptação/Hipertrofia", methods: "Método de execução Simples", reps: "10", volume: "16 séries/grupo", intensity: "70-75% 1RM", recovery: "30s" },
        { week: "3ª e 4ª Semanas", phase: "Adaptação/Hipertrofia", methods: "Método de execução Simples", reps: "8-9", volume: "16 séries/grupo", intensity: "70-75% 1RM", recovery: "30s" },
        { week: "5ª e 6ª Semanas", phase: "Força", methods: "Método de execução Simples + Rest-Pause", reps: "6-7", volume: "14 séries/grupo", intensity: "80-85% 1RM", recovery: "30s" },
        { week: "7ª e 8ª Semanas", phase: "Força", methods: "Método de execução Simples + Rest-Pause", reps: "5-6", volume: "12 séries/grupo", intensity: "80-85% 1RM", recovery: "30s" },
        { week: "9ª e 10ª Semanas", phase: "Força Máxima", methods: "Método de execução Simples + Pirâmide", reps: "3 / 4 / 5", volume: "10 séries/grupo", intensity: "85-90% 1RM", recovery: "30s" },
        { week: "11ª Semana", phase: "Força Máxima", methods: "Método de execução Simples + Rest-Pause + Pirâmide", reps: "3 / 4 / 5", volume: "10 séries/grupo", intensity: "85-90% 1RM", recovery: "30s" },
        { week: "12ª e 13ª Semanas", phase: "Deload", methods: "Método de execução Simples (recuperação)", reps: "6-8", volume: "8 séries/grupo", intensity: "50-60% 1RM", recovery: "30s" }
    ];
...
function renderCalendar(email) {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        if (dayOfWeek === 1 || dayOfWeek === 4) { // Segunda e Quinta
            classes += ' treino-A';
            workoutType = 'A';
        } else if (dayOfWeek === 2 || dayOfWeek === 5) { // Terça e Sexta
            classes += ' treino-B';
            workoutType = 'B';
        }
...
function processExercises(exercises, email) {
    ...
    const startDate = exercises[0].startDate;
    if (!startDate) return exercises.map((ex, index) => ({ ...ex, name: `${index + 1}. ${ex.name}` }));

    const currentWeek = getWeekNumber(startDate);
    const periodizacao = database.trainingPlans.periodizacao;
